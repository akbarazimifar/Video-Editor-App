...
import static com.hoanganhtuan95ptit.fillter.FilterFragment.filterView;
...
class FilterView extends AppCompatImageView {
    ...
    public void filterbitmapdemo(Bitmap bitmap, FilterManager.FilterType type)
    {
        if (this.type == type) return;
        this.type = type;
        ImageEglSurface imageEglSurface = new ImageEglSurface(bitmap.getWidth(), bitmap.getHeight());
        ...
        imageEglSurface.release();
        imageRenderer.destroy();
        FilterFragment.setimage(bitmap);

       // return bitmap;

    }
    public void savebitmap(Bitmap bitmap, FilterManager.FilterType type)
    {
        ImageEglSurface imageEglSurface = new ImageEglSurface(bitmap.getWidth(), bitmap.getHeight());
        imageEglSurface.setRenderer(filterView.getImageRenderer());
        filterView.getImageRenderer().changeFilter(type);
