...
public class PhotoUtil {
    ...
    public static Rect getRec(Rect srcRect, int bitmapWidth, int bitmapHeight, float toWidth, float toHeight) {
        if (srcRect == null) {
            srcRect = new Rect();
        }
        float rate = toWidth / toHeight;
     ...
     * @return
     */
    public static Rect getFitXyRect(Rect srcRect, int bitmapWidth, int bitmapHeight, float toWidth, float toHeight) {
        if (srcRect == null) {
            srcRect = new Rect();
        }
        float rate = toWidth / toHeight;
        ...
        Log.d("gg","rate : "+rate);
        Log.d("gg","bitmapRate : "+bitmapRate);
        if (Math.abs(rate - bitmapRate) < 0.01) {
